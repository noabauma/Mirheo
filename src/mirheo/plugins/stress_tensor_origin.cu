...
__global__ void totalStress(PVview view, const real mass, stress_tensor_plugin::ReductionType *StressTensor)
{
    ...
    if (tid < N){
        ...
        P.xx = mass*v.x*v.x + r.x*f.x;
        P.xy = mass_xy + r.x*f.y;
        P.xz = mass_xz + r.x*f.z;
        P.yx = mass_xy + r.y*f.x;
        P.yy = mass*v.y*v.y + r.y*f.y;
        P.yz = mass_yz + r.y*f.z;
        P.zx = mass_xz + r.z*f.x;
        P.zy = mass_yz + r.z*f.y;
        P.zz = mass*v.z*v.z + r.z*f.z;
    }
    ...
    P.yx = warpReduce(P.yx, sum);
    P.yy = warpReduce(P.yy, sum);
    P.yz = warpReduce(P.yz, sum);
    P.zx = warpReduce(P.zx, sum);
    P.zy = warpReduce(P.zy, sum);
    ...
    if (laneId() == 0){
        ...
        atomicAdd(&StressTensor->yx, P.yx);
        atomicAdd(&StressTensor->yy, P.yy);
        atomicAdd(&StressTensor->yz, P.yz);
        atomicAdd(&StressTensor->zx, P.zx);
        atomicAdd(&StressTensor->zy, P.zy);
...
void StressTensorDumper::setup(const MPI_Comm& comm, const MPI_Comm& interComm)
{
    ...
    assert(mask_.size() == 9 && "example mask = 011101010 -> Pxy,Pxz,Pyx,Pyz,Pzy");
...
void StressTensorDumper::deserialize()
{
    ...
    */
    ...
    fprintf(fdump_.get(), "%g,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e\n", curTime, totalStress.xx, totalStress.xy, totalStress.xz, totalStress.yx, totalStress.yy, totalStress.yz, totalStress.zx, totalStress.zy, totalStress.zz);
}
